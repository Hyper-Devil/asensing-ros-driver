...
ros::Time convertGPSTimeToROSTime(int gpsWeek, double gpsSec)
{
  ...
  double totalSeconds = static_cast<double>(GPS_TO_UNIX_SECONDS - LEAP_SECONDS) + static_cast<double>(gpsWeek) * WEEK_SECONDS + gpsSec;
      ...
      if (ser.isOpen())
      {
        ...
        if (!read.empty())
        {
          ...
          while (input.length() >= 3)
          {
            ...
            float yawf = static_cast<float>(M_PI) / 2.0f - static_cast<float>(yaw_raw) * (360.0f / 32768.0f) * (static_cast<float>(M_PI) / 180.0f);
            while (yawf > static_cast<float>(M_PI))
              yawf -= 2.0f * static_cast<float>(M_PI);
            while (yawf < -static_cast<float>(M_PI))
              yawf += 2.0f * static_cast<float>(M_PI);
